...
def refToBibtex(id, reference, title, newLineFlag):
    tagList = ['surname', 'article-title', 'source', 'editor', 'publisher', 'volume',
               'issue', 'page', 'year', 'other', 'identifier', 'url']
    newLine = "\n"
...
def tagging(ln, label, prob):  # with probability
    ...
    for i in range(len(ln)):
        ...
        if bool(tmp1):
            tmp1 = (
                tmp1[0][0], tmp1[-1][1])  # si le text est dévisé en deux en prend le text entier test avec **ze.yd**
            if not b:
                a = abv.index(label[i])
                if a == 3:
                ...
                if ((tmp == 3) & (a != 3)):
                    nln = nln + '</' + tag[a] + '>' + old + ' ' + ln[i][0:tmp1[0]] + '<' + tag[tmp] + ' prob="' + str(
                        prob[i]) + '">' + ln[i][tmp1[0]::]
                    old = ''
                # v=1
        ...
        if bool(tmp1):
            ...
            if not b:
                ...
                if ((tmp == 3) & (a == 3) & (tmpn != 3)):
                    ...
                    if ((tmp == 3) & (a != 3)):
                        nln = nln + '</' + tag[a] + '>' + old + ' ' + ln[i][0:tmp1[0]] + '<' + tag[tmp] + '>' + ln[i][
                                                                                                                tmp1[
                                                                                                                    0]::]
                        old = ''
                    # v=1
